...
class DolfinVectorSpace(object):
    ...
    def __inner_obj(x, y):
        if isinstance(x, GenericFunction):            
            assert isinstance(y, GenericFunction)        
            return assemble(inner(x, y)*dx)         #L^2 inner product 
            #return x.vector().inner(y.vector())    #\ell^2 inner product 
            
            # alternatively, but not working with mpi
            #return numpy.inner(x.vector().array(), y.vector().array())     #\ell^2 inner product 
    ...
    def copy(x, y):
        if isinstance(x, list):
            ...
            [DolfinVectorSpace.__assign_obj(x, y[0])]
    ...
    def scal(alpha, x):
        if isinstance(x, list):
            ...
            [DolfinVectorSpace.__scale_obj(alpha, x)]
...
try:
    ...
    class OptizelleObjective(Optizelle.ScalarValuedFunction):
        ...
        def grad(self, x, grad):
            ...
            J
            print "j_profit =", -J
            #if -J > 7.58*1e06: sys.exit(0)                   
            #else: 
            #    pass                
            out = self.rf.derivative(forget=False, project=True)
            #import ipdb; ipdb.set_trace()
            #print "norm(dj) =", norm(out[0])
